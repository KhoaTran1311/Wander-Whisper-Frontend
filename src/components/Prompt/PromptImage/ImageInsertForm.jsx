    ...
    return (
		<>
			...
			>
				...
				>
					<div className={`flex flex-col bg-zinc-900 border border-zinc-700/30 transition-[height] ${!images.length ? "h-98" : "h-78"} w-112 sm:w-132 md:w-168 rounded-2xl p-6 gap-3 shadow-xl`}>
					...
					<ImageViewer fileInputRef={fileInputRef}/>
				</div>
			</div>
			<Toast message={`Successfully added ${numImgAdded} image`} onClose={() => setNumImgAdded(-1)} notify={numImgAdded}/>
		</>
    )
}
