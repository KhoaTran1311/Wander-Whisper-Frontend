...
const ImageInsertForm = () => {
    const { images, setImages, open, setOpen } = useImageContext();
    ...
    return (
        <div
			className={`fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/50 transition-opacity backdrop-blur-xl p-20 ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
					...
					<div 
						...
						>
							...
							<div className={`rounded-full transition-colors ${isDragOver ? "bg-blue-800" : "bg-zinc-800"} p-3 shadow-sm`}>
								<div className={`${isDragOver ? "text-blue-500" : "text-neutral-500"}`}>
									<ImagePlusIcon/>
								</div>
							</div>
							<div className='select-none transition-colors text-center'>
								<p className={`text-md ${isDragOver ? "text-blue-200" : "text-neutral-200"}`}>Click to select</p>
								<p className={`text-sm ${isDragOver ? "text-blue-500" : "text-neutral-500"}`}>
